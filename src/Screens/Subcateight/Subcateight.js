...
class Subcateight extends Component {
  constructor(props) {
    super(props);
    this.state = {
      MainArray8: [],
  ...
  componentDidMount = () => {
    const {MainArray7} = this.props.route.params;
    const {myid} = this.props.route.params;
    const {my_token} = this.props;
    console.log(my_token, 'access token at subcatone page');
    const {MainArray8} = this.state;
    const {checkarrayeight} = this.state;
    console.log(MainArray7, 'at subcatsix');
    ...
    actions
      ...
      .then(res => {
        ...
        checkarrayeight.push(res);
        console.log(checkarrayeight, 'aaaaaaaaaaaaaaa');
        this.setState({
          MainArray8: checkarrayeight,
        });
      })
      .catch(err => {
  ...
  onClickDiseasesThree = id => {
    ...
    navigation.navigate(navigationStrings.SUBCATNINE, {
      MainArray8: MainArray8,
      myid: myid,
    });
  };
  renderItem = ({item}) => {
    ...
    return (
      <View>
        ...
        <View>{/* <Text>subcat eight</Text> */}</View>
  ...
  render() {
    const {MainArray8} = this.state;
    const {checkarrayeight} = this.state;
    console.log(MainArray8, 'at subcategory 6 page');
    return (
      <View>
        <View>
          <Header />
...
export default connect(mapStateToProps)(Subcateight);
